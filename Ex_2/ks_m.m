function [tsave, xsave, usave] = ks_m(u,N)
...
% u = -sin(x)+2*cos(2*x)+3*cos(3*x)-4*sin(4*x); %Initial condition
% (substituted with random initialization)
...
%cutoff = cutoff & tt<130;
 cutoff = tt > 0; cutoff = cutoff & tt<0.62;
